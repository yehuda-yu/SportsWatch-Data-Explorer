...
def get_chart(df,parameter):
    ...
    fig = px.line(df, x=df_filtered.index, y=parameter, title=f"{parameter} Over Time")
            ...
            dict(
                ...
                buttons=list([
                    dict(
                        label="Last Week",
                        method="update",
                        args=[{"visible": [True if (df.index.max() - pd.Timedelta(days=7)) <= d <= df.index.max() else False for d in df.index]}]
                    ),
                    dict(
                        label="Last Month",
                        method="update",
                        args=[{"visible": [True if (df.index.max() - pd.Timedelta(days=30)) <= d <= df.index.max() else False for d in df.index]}]
                    ),
                    dict(
                        label="Last 6 Months",
                        method="update",
                        args=[{"visible": [True if (df.index.max() - pd.Timedelta(days=180)) <= d <= df.index.max() else False for d in df.index]}]
                    ),
                    dict(
                        label="Last Year",
                        method="update",
                        args=[{"visible": [True if (df.index.max() - pd.Timedelta(days=365)) <= d <= df.index.max() else False for d in df.index]}]
                    ),
                    dict(
                        label="All Data",
                        method="update",
                        args=[{"visible": [True for d in df.index]}]
                    )
                ])
            )
        ]
...
def analyze_activity_duration(df, duration_column):
    ...
    try:
        ...
        fig = px.histogram(df_minutes, x=duration_column,nbins=20,title='Distribution of Activity Durations',labels={'x': 'Activity Duration (minutes)', 'y': 'Frequency'},)
        st.plotly_chart(fig)
        """
        # Create a histogram using Plotly
        fig = px.histogram(df_minutes, x=duration_column, nbins=20, labels={'x': 'Activity Duration (minutes)', 'y': 'Frequency'},
                           title='Distribution of Activity Durations', marginal='kde')
